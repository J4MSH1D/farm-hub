...
<template>
  <div class="container">
    <h2 class="text-2xl text-zinc-900 font-semibold">{{ $t("Объявления о товарах") }}</h2>

    <div class="flex items-center justify-between gap-5">
      <div class="flex gap-3 mt-5">
        <a-radio-group size="large" v-model:value="activeTab">
          <a-radio-button size="large" value="published">{{ $t("Опубликованные") }}</a-radio-button>
          <a-radio-button size="large" value="requests">{{ $t("Мои отклики и запросы ") }}</a-radio-button>
        </a-radio-group>
      </div>
